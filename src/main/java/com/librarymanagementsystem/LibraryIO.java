...
public class LibraryIO {
    ...
    public static List<LibraryItem> loadItemFromFile(String filName) {
    ...
    public static void saveUserListToFile(List<User> users, String fileName) {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(fileName))) {
            ...
            System.out.println("User List save to the file successfully!");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static Map<String, String> oadBorrowedItemsFromFile(String fileName) {
        File file = new File(fileName);
        if (!file.exists()) {
